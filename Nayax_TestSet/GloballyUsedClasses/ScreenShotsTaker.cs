    ...
    class ScreenShotsTaker
    {
        ...
        public static void TakeScreenShot(string ShotName, IWebElement elementToScroll_) {
            ...
            RunTask = Task.Run(() => {
                ...
                if (GloballyUsedClasses.GetElementDimensions.OffsetHeight_ != 0)
                    { NumberOfShots = GloballyUsedClasses.GetElementDimensions.ScrollHeight_ / GloballyUsedClasses.GetElementDimensions.OffsetHeight_ ; }
                else
                    { NumberOfShots = 1; }//if
                ...
                for (int i = 0; i < NumberOfShots; i++) {
                    ...
                    ((IJavaScriptExecutor)TestProcedure.webDriver).ExecuteScript("arguments[0].scrollTo(0," + ScrollStep + ");", elementToScroll_);

                    // forced pause
                    System.Threading.Thread.Sleep(Convert.ToInt32(GloballyUsedClasses.BandwidthCheck.DownloadRate * 10));

                    // takes a screenshot
                    Screenshot ScreenShot = ((ITakesScreenshot)TestProcedure.webDriver).GetScreenshot();
                    ...
                    // increasing step
                    ScrollStep += GloballyUsedClasses.GetElementDimensions.OffsetHeight_;
        ...
        public static void DrawBorderAroundElementByCssSelector(string SelectorType, string Selector_, string SingleOrPlural_, string ItemIndex_)
        {
            ...
            RunTask = Task.Run(() => {
                ...
                ((IJavaScriptExecutor)TestProcedure.webDriver).ExecuteScript("document.getElement" + SingleOrPlural_ + "By" + SelectorType + "(\"" + Selector_ + "\")" + ItemIndex_ + ".style.border=\"2px solid " + Asserts.TestReportFontColor + "\"");
        ...
        public static void DrawBorderAroundElementByXpath(string Xpath_)
        {
            ...
            RunTask = Task.Run(() => {
                ...
                ((IJavaScriptExecutor)TestProcedure.webDriver).ExecuteScript("document.evaluate(\"" + Xpath_ + "\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).style.border=\"2px solid " + Asserts.TestReportFontColor + "\"\"");
